...
public interface TeamRepository extends CrudRepository<Team, String>   {
    ...
    @Query(value = "select count(id) from game where id in (select games_id from game_teams where teams_name like %?1) and winner like %?1 UNION select count(id) from game where id in (select games_id from game_teams where teams_name like %?1) and winner not like %?1 UNION select count(id) from game where id in (select games_id from game_teams where teams_name like %?1)", nativeQuery = true)
